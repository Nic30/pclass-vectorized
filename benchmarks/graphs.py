...
def generate_graph_construction_time_for_ruleset(ruleset_order, app_data, x, file_name):
    ...
    for name, data in app_data.items():
        y = [ d[2] / 1000.0 for d in sorted(data, key=lambda d: ruleset_order[d[0]])]
        line1, = ax.plot(x, y, label=name)
...
def generate_graphs_depending_on_ruleset_size(db_file_name, number_of_flows):
    """
    [y]  |
         |-----
             [rulesets ordered by decreasing lookup speed]
    ...
    data2 = [(d[0], d[1]) for d in sorted(list(app_data.values())[0], key=lambda d: d[1], reverse=False)]
    ruleset_order_by_size = {d[0] : len(data2) - i for i, d in enumerate(data2)}
    ...
    generate_graph_construction_time_for_ruleset(
        ...
        "fig/construction_time_for_increasing_ruleset_complexity.png")

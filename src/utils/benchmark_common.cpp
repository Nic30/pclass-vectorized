...
void BenchmarkStats::dump(std::function<void(std::ostream &)> json_extra,
	...
	auto lookup_speed = ((LOOKUP_CNT / double(lookup_time_from_packets))
			* 1000.0);
	...
	out << "\"minimal_benchmark_overhead\":"
			<< (lookup_timer->us() * 1000 / double(lookup_time_from_packets))
			<< "," << std::endl; // (1 = zero overhead caused by precise timers)
